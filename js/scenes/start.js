...
export default class StartScene extends Phaser.Scene
{
  ...
  create ()
  {
    ...
    // Creamos el mapa.
    this.map = this.make.tilemap({ key: "start-map" });
    ...
    this.UI.characterSheet.startCharacter(
      {
        name: "Player",
        level: 1,
        xp: 0,
    ...
    const playerSpawn = this.map.findObject("player", obj => obj.name === "playerSpawn");
    ...
    this.enemyManager.addStartEnemies();


    // Creamos un grupo para contener todos los cofres del mapa.
    this.chests = this.add.group();
    // Agregamos todos los cofres del mapa base.
    this.addChests(this.map);
    ...
    const levelEnd = this.map.findObject("player", obj => obj.name === "levelEnd");
